...
### MT5 en Linux
- El paquete `MetaTrader5` es Windows-only, así que el terminal corre bajo **Wine** (dentro de `WINEPREFIX`, con display virtual `Xvfb`), y el bot se conecta vía el puente **`mt5linux`** (servidor corriendo con el Python de Wine, cliente nativo en el proceso Linux). Todo el resto del pipeline (pandas, XGBoost, etc.) corre nativo en Linux.
- `core/mt5_connector.py` (clase `MT5Connector`) lanza el terminal vía `subprocess.Popen(["wine", mt5_exe])` si no está abierto.
- `core/mt5_compat.py` centraliza el import: intenta `MetaTrader5` nativo/mockeado primero, y solo si falla cae al cliente `mt5linux`. Todos los módulos que hablan con MT5 (`mt5_connector`, `mt5_stream`, `trade_manager`, `data_downloader`) importan `mt5` desde ahí — nunca `import MetaTrader5` directo.
- Credenciales en `.env`: MT5_LOGIN, MT5_PASSWORD, MT5_SERVER, DISPLAY, WINEPREFIX, MT5_EXE, MT5_BRIDGE_HOST, MT5_BRIDGE_PORT.
