"""
mt5_compat.py
Cross-platform access point for the MetaTrader5 API.
...
on Linux/macOS. On those platforms the real terminal + its Python API run
inside Wine, and this process connects to them through `mt5linux`
...
`import MetaTrader5 as mt5`, so the same code runs unmodified on Windows and
on Linux, and test suites that mock `sys.modules["MetaTrader5"]` keep working
